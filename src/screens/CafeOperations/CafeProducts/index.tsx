...
import { getCafeProducts } from '../../../api/cafe';
...
const CafeProducts = () => {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const res = await getCafeProducts({ branch_id: branchId, limit: 200 });
  ...
  const renderRow = ({ item, index }: { item: any; index: number }) => (
    <View style={[styles.card, index % 2 === 1 && { backgroundColor: '#FBF8F8' }]}>
      <View style={styles.iconBox}>
        <Icon name="food-variant" size={20} color="#E63946" />
      </View>
      ...
      <Text style={styles.price}>{fmtRs(item.price)}</Text>
    </View>
  );
      ...
      <View style={styles.body}>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.center}>
                <Icon name="food-off" size={48} color="#ddd" />
                <Text style={styles.emptyTitle}>No Products</Text>
                <Text style={styles.emptyText}>No cafe products found.</Text>
              </View>
            }
...
const styles = StyleSheet.create({
  ...
  category:    { fontSize: 12, color: '#E63946', fontWeight: '500' },
