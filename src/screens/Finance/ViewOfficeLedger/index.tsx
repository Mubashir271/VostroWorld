...
import { getOfficeCashLedger, getOfficeCashBalance, deleteOfficeCashEntry } from '../../../api/employeeDashboard';
...
const ViewOfficeLedger = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [ledgerRes, balRes] = await Promise.all([
        getOfficeCashLedger({ branch_id: branchId, start_date: startDate, end_date: endDate, limit: 500 }),
        getOfficeCashBalance(branchId),
      ]);
      ...
      setCurrentBalance(typeof balRes?.totalBalance === 'number' ? balRes.totalBalance : parseFloat(balRes?.totalBalance ?? 0) || 0);
      setFetched(true);
    } catch (e: any) {
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteOfficeCashEntry(id);
      flash('Entry deleted.');
      load();
    } catch {
  ...
  const totalDebit = rows.reduce((s, r) => s + (r._debit ?? 0), 0);
  const totalCredit = rows.reduce((s, r) => s + (r._credit ?? 0), 0);

  return (
    <View style={styles.root}>
      <AppHeader
        title="Office Cash Ledger"
      ...
      <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          <View style={styles.cardTitleRow}>
            ...
            <TouchableOpacity style={styles.addBtn} onPress={() => navigation.navigate('AddOfficeCash')}>
              <Icon name="plus" size={14} color="#fff" />
              <Text style={styles.addBtnText}>Add Office Cash</Text>
            </TouchableOpacity>
          </View>
        ...
        {(fetched || loading) && (
          <View style={styles.card}>
            <View style={styles.cardTitleRow}>
              <Text style={styles.cardTitle}>Office Cash Ledger</Text>
                  ...
                  <>
                    ...
                    <View style={styles.summaryBox}>
                      <View style={styles.summaryRow}>
                        ...
                        <Text style={styles.summaryValue}>{Rs(totalDebit)}</Text>
                      </View>
                      <View style={styles.summaryRow}>
                        <Text style={styles.summaryLabel}>Total Credit:</Text>
                        <Text style={styles.summaryValue}>{Rs(totalCredit)}</Text>
                      </View>
                      {currentBalance !== null && (
...
export default ViewOfficeLedger;
