...
import { getPhysioPrescriptions } from '../../../api/physio';
...
const PhysiotherapyPrescriptions = () => {
  ...
  const load = useCallback(async (targetPage: number, isRefresh = false) => {
    ...
    try {
      const res = await getPhysioPrescriptions({
        branch_id: branchId,
        limit: PAGE_SIZE,
        page: targetPage,
  ...
  const handleAddPrescription = () => {
    navigation.navigate('AddPhysioPrescription');
  };

  return (
    <View style={styles.container}>
      <AppHeader
        title="Physio Prescriptions"
      ...
      <View style={styles.toolbar}>
        <View style={styles.searchBar}>
          ...
          <TextInput
            ...
            placeholder="Client, diagnosis, region..."
        ...
        <View style={styles.filterRow}>
          ...
          <TouchableOpacity style={styles.addAction} onPress={handleAddPrescription}>
        ...
        >
          <ScrollView horizontal showsHorizontalScrollIndicator>
            <View>
              ...
              {list.length === 0 ? (
                <View style={styles.noRecord}><Text style={styles.noRecordText}>No prescriptions found</Text></View>
              ) : (
                list.map((p, i) => (
                  <View key={p.id} style={[tbl.dataRow, i % 2 === 1 && tbl.dataRowAlt]}>
                    <Text style={[tbl.cell, tbl.cellMuted, { width: 34 }]}>{(page - 1) * PAGE_SIZE + i + 1}</Text>
                    <Text style={[tbl.cell, { width: 70 }]}>{fmtDDMMYY(p.prescription_date)}</Text>
                    <Text style={[tbl.cell, tbl.cellRed, { width: 140 }]} numberOfLines={1}>{p.client_name ?? '—'}</Text>
                    ...
                    <Text style={[tbl.cell, { width: 120 }]} numberOfLines={1}>{p.physio_name ?? '—'}</Text>
                    <Text style={[tbl.cell, { width: 140 }]} numberOfLines={1}>{p.notes ?? '—'}</Text>
                  </View>
                ))
              )}
            </View>
...
export default PhysiotherapyPrescriptions;
