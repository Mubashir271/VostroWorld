...
import { getSPTAppointments, getSPTAppointmentTrainers } from '../../../api/employeeDashboard';

// Confirmed live 2026-07-01 via HAR — endpoint is `index-small-pt`, trainer
// dropdown sends `is_gx_trainer=` (empty, all PTs). Default date range in the
// web admin is ~1 month back to ~1 week ahead. All schedule[] arrays were
// empty (no SPT bookings exist yet), so cell content when booked is inferred
// from the GXAppointments pattern (client_name).

interface Trainer { id: number; name: string; }
interface DayBooking { order_id: number; client_name: string; package_name: string; session_count: number; }
interface TimeSlotRow { time_slot_assignment_id: number; time: string; schedule: DayBooking[] | Record<string, DayBooking>; }
interface TrainerBlock { trainer_id: number; name: string; branch_name: string; time_slots: TimeSlotRow[]; }
...
const defaultStart = () => { const d = new Date(); d.setDate(d.getDate() - 28); return fmt(d); };
const defaultEnd = () => { const d = new Date(); d.setDate(d.getDate() + 6); return fmt(d); };
...
const SPTAppointments = () => {
  ...
  const load = useCallback(async (targetPage = 1) => {
    ...
    setError('');
    try {
      const res = await getSPTAppointments({
        branch_id: branchId,
      ...
      const data: TrainerBlock[] = res?.data ?? [];
      ...
      if (s === 404 || s === 422) { setBlocks([]); setTotalPages(1); setFetched(true); }
      else setError(e?.response?.data?.message ?? 'Failed to load SPT appointments.');
    } finally {
      setLoading(false);
    }
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
        <View style={styles.card}>
          <Text style={styles.cardTitle}>SPT Appointments</Text>
                    ...
                    {block.time_slots.map((slot, i) => (
                      <View key={slot.time_slot_assignment_id} style={[styles.tr, i % 2 === 1 && styles.trAlt]}>
                        ...
                        {DAYS.map(day => {
                          ...
                          return (
                            <Text key={day} style={[styles.td, styles.colDay, booking ? styles.bookedText : styles.freeText]}>
...
const styles = StyleSheet.create({
  ...
  blockHeader: {
    ...
    backgroundColor: '#1A1A1A', paddingVertical: 10, paddingHorizontal: 10,
