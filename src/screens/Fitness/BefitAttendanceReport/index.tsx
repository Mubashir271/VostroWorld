...
import { getHRSessionsAll, getGXTrainers } from '../../../api/employeeDashboard';

// NOT CONFIRMED LIVE: `hr/sessions` (the only session-attendance endpoint
// found so far — see GXAttendanceReport for how it was confirmed for GX/PT)
// was checked across 26k+ rows on two branches and never returned a
// `type` value other than `GX` or `PT`. There is no known endpoint yet for
// Befit attendance; `type === 'Befit'` below is a placeholder filter that
// will most likely return zero rows until the real endpoint is found
// (e.g. from the web admin's Network tab on this exact page).
...
const COLS = [
  ...
  { key: 'staff', label: 'Trainer Attendance', width: 110 },
...
// `type` filter for this category — see NOT CONFIRMED LIVE note above.
const CATEGORY_TYPE = 'Befit';
const SCREEN_TITLE = 'Befit Attendance Report';
...
const BefitAttendanceReport = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(data.filter(r => r.type === CATEGORY_TYPE));
      setFetched(true);
    } catch (e: any) {
      const status = e?.response?.status;
      if (status === 404 || status === 422) { setRows([]); setFetched(true); }
      else setError(e?.response?.data?.message || `Failed to load ${SCREEN_TITLE.toLowerCase()}.`);
    } finally {
      setLoading(false);
    }
  ...
  const visibleRows = rows.filter(r => {
    ...
    if (packageFilter.trim() && !(r.package_name ?? '').toLowerCase().includes(packageFilter.trim().toLowerCase())) return false;
    if (staffStatus !== 'All' && r.staff_status !== staffStatus) return false;
    if (clientStatus !== 'All' && r.client_status !== clientStatus) return false;
    return true;
  });

  useEffect(() => { setPage(1); }, [trainerId, packageFilter, staffStatus, clientStatus, rows]);
  ...
  const stats = [
    ...
    { label: 'Trainer Delivered', value: visibleRows.filter(r => r.staff_status === 'Delivered').length, color: '#10b981' },
    { label: 'Client Delivered', value: visibleRows.filter(r => r.client_status === 'Delivered').length, color: '#10b981' },
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          ...
          <Text style={[styles.label, { marginTop: 12 }]}>Client Attendance</Text>
          <View style={styles.pillRow}>
            {STATUS_OPTIONS.map(s => (
                      ...
                      <View style={{ width: TABLE_W }}>
                        ...
                        {pagedRows.map((r, i) => (
                          <View key={r.id} style={[styles.tr, i % 2 === 1 && styles.trAlt]}>
                            ...
                            <Text style={[styles.td, { width: COLS[7].width }]}>{r.staff_status ?? '-'}</Text>
                            <Text style={[styles.td, { width: COLS[8].width }]}>{r.client_status ?? '-'}</Text>
                            <Text style={[styles.td, { width: COLS[9].width }]}>-</Text>
                            <Text style={[styles.td, { width: COLS[10].width }]}>-</Text>
                            <Text style={[styles.td, { width: COLS[11].width, color: '#1565C0', fontWeight: '700' }]}>Verify</Text>
                            <Text style={[styles.td, { width: COLS[12].width, color: '#2E7D32', fontWeight: '700' }]}>Active</Text>
                          </View>
                        ))}
                      </View>
                    </ScrollView>
...
export default BefitAttendanceReport;
