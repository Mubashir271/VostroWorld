...
// Appointments" page, including a real (non-empty) response. Despite
// PROJECT_STATUS.md noting the plain `trainer-schedule/index` endpoint
// returns empty schedules for GX trainers (hence `index-gx` existing as a
// separate route), Befit bookings live in this same general table and
// `index` (= the already-wired `getTrainerSchedule`, same as
// `TrainerAppointments`) returns them correctly — no new schedule endpoint
// needed. Response shape confirmed identical to `TrainerAppointments`'s,
// down to the `pt_start_date`/`pt_end_date` fields on `schedule`. Only the
// trainer dropdown source differs — same `auth/get-name` (designation_id=1,
// is_gx_trainer empty = every PT, not just GX-flagged) already wired as
// `getBefitBookingTrainers` for Befit Bookings.
...
const BefitAppointments = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const filtered = trainerId ? data.filter(b => String(b.trainer_id) === trainerId) : data;
      setBlocks(Array.isArray(filtered) ? filtered : []);
      setFetched(true);
    } catch (e: any) {
      const status = e?.response?.status;
      if (status === 404 || status === 422) {
        setBlocks([]); setFetched(true);
      } else {
        setError(e?.response?.data?.message || 'Failed to load Befit appointments.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.body} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          <Text style={styles.cardTitle}>Befit Appointments</Text>
...
export default BefitAppointments;
