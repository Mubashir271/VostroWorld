...
// Small PT Group Packages category code
const CATEGORY = 4;
...
const mapPackage = (p: any): SmallPTGroupItem => ({
  id: p.id,
  ...
  sessionTime: p.start_time && p.end_time ? `${p.start_time} / ${p.end_time}` : '',
  group: p.group_name ?? '',
...
const SmallPTGroupPackages = () => {
  ...
  const handleAdd = () => {
    if (!trainer) {
      Alert.alert('Validation', 'Please select an available trainer.');
      return;
    }
    if (!name.trim()) {
    ...
    if (!sessions.trim()) {
      Alert.alert('Validation', 'Please enter the number of sessions.');
      return;
    }
    if (!duration.trim()) {
    ...
    const sessionTime = startTime && endTime ? `${displayTime(startTime)} / ${displayTime(endTime)}` : '';
    if (editingId) {
      setPackages(prev => prev.map(p => p.id === editingId
        ? { ...p, branch, name: name.trim(), trainer: trainerLabel(trainer), sessions: sessions.trim(), duration: duration.trim(), price: price.trim(), sessionTime: sessionTime || p.sessionTime }
        : p));
      setEditingId(null);
    } else {
      setPackages(prev => [...prev, {
        id: Date.now(), branch, name: name.trim(), trainer: trainerLabel(trainer), sessions: sessions.trim(), duration: duration.trim(), price: price.trim(), sessionTime, group: '', active: true,
      }]);
    }
    resetForm();
  };

  const handleUpdate = (item: SmallPTGroupItem) => {
    setEditingId(item.id);
    setBranch(item.branch);
    setTrainer(trainers.find(t => trainerLabel(t) === item.trainer) ?? { full_name: item.trainer });
    setName(item.name);
  ...
  const handleSetActive = (item: SmallPTGroupItem, active: boolean) => {
    Alert.alert(
  ...
  const handleDelete = (item: SmallPTGroupItem) => {
  ...
  const trainerOptions = trainers.map(t => ({ id: String(t.id), label: trainerLabel(t) }));

  return (
    <View style={styles.container}>
      <AppHeader
        title="Small PT Group Packages"
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {/* ── Add / Update Small PT Group ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>{editingId ? 'Update Small PT Group' : 'Small PT Group'}</Text>
          </View>
          <View style={styles.form}>
            ...
            <SelectionField
              ...
              value={trainer ? trainerLabel(trainer) : ''}
            ...
            <View style={styles.row2}>
              <View style={[styles.fieldContainer, { flex: 1 }]}>
                ...
                <TouchableOpacity style={styles.timeBox} onPress={() => setPickerFor('start')}>
                  <Text style={styles.timeText}>{startTime ? displayTime(startTime) : '12:30 PM'}</Text>
                  <Icon name="clock-outline" size={16} color="#888" />
                </TouchableOpacity>
              </View>
              <View style={[styles.fieldContainer, { flex: 1 }]}>
                <Text style={styles.label}>End Time:</Text>
                <TouchableOpacity style={styles.timeBox} onPress={() => setPickerFor('end')}>
                  <Text style={styles.timeText}>{endTime ? displayTime(endTime) : '12:30 PM'}</Text>
                  <Icon name="clock-outline" size={16} color="#888" />
                </TouchableOpacity>
              </View>
            </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Active PT Package</Text>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator>
            <View>
              <View style={tbl.headerRow}>
                ...
                <Text style={[tbl.headerCell, { width: 80 }]}>Session Count</Text>
                <Text style={[tbl.headerCell, { width: 90 }]}>Duration</Text>
                <Text style={[tbl.headerCell, { width: 90 }]}>Price</Text>
                <Text style={[tbl.headerCell, { width: 130 }]}>Session Time</Text>
              ...
              {loading
                ...
                : activePageData.map((p, i) => (
                  <View key={p.id} style={[tbl.dataRow, (activeStartIdx + i) % 2 === 1 && tbl.dataRowAlt]}>
                    ...
                    <Text style={[tbl.cell, { width: 120 }]} numberOfLines={1}>{p.trainer}</Text>
                    <Text style={[tbl.cell, { width: 80 }]} numberOfLines={1}>{p.sessions}</Text>
                    ...
                    <Text style={[tbl.cell, { width: 130 }]} numberOfLines={1}>{p.sessionTime}</Text>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Inactive PT Package</Text>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator>
            <View>
              <View style={tbl.headerRow}>
                ...
                <Text style={[tbl.headerCell, { width: 90 }]}>Price</Text>
                <Text style={[tbl.headerCell, { width: 130 }]}>Session Time</Text>
                <Text style={[tbl.headerCell, { width: 100 }]}>Group</Text>
              ...
              {loading
                ...
                : inactivePageData.map((p, i) => (
                  <View key={p.id} style={[tbl.dataRow, (inactiveStartIdx + i) % 2 === 1 && tbl.dataRowAlt]}>
                    ...
                    <Text style={[tbl.cell, { width: 90 }]} numberOfLines={1}>{`Rs ${p.price}/-`}</Text>
                    <Text style={[tbl.cell, { width: 130 }]} numberOfLines={1}>{p.sessionTime}</Text>
                    <Text style={[tbl.cell, { width: 100 }]} numberOfLines={1}>{p.group}</Text>
      ...
      <SelectionModal
        ...
        selectedValue={trainer ? trainerLabel(trainer) : ''}
        onSelect={(val: string) => {
          setTrainer(trainers.find(t => trainerLabel(t) === val) ?? null);
...
const styles = StyleSheet.create({
  ...
  timeBox:          { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#FAFAFA', borderRadius: 8, padding: 14, borderWidth: 1, borderColor: '#E0E0E0' },
...
export default SmallPTGroupPackages;
