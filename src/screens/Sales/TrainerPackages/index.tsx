...
const TRAINER_OPTIONS = [
  ...
  { id: '2', label: 'Bilal Sheikh' },
  { id: '3', label: 'Sara Malik' },
  { id: '4', label: 'Usman Tariq' },
];
...
// PT/Trainer Packages category code
const CATEGORY = 2;

const mapPackage = (p: any): TrainerPackageItem => ({
  id: p.id,
  branch: p.branches_name ?? '',
  name: p.package_name ?? '',
  trainer: [p.user_first_name, p.user_last_name].filter(Boolean).join(' '),
  trainingType: p.trainer_membership_name ?? '',
...
const TrainerPackages = () => {
    ...
    const load = async () => {
      ...
      try {
        const [activeRes, inactiveRes] = await Promise.all([
          getPackages({ branch_id: branchId, category: CATEGORY, status: 1 }),
          getPackages({ branch_id: branchId, category: CATEGORY, status: 0 }),
        ]);
        if (cancelled) return;
  ...
  const [modalConfig, setModalConfig] = useState<{ visible: boolean; field: 'branch' | 'trainer' | 'trainingType' | null }>({ visible: false, field: null });
  ...
  const handleAdd = () => {
    ...
    if (!trainingType) {
      Alert.alert('Validation', 'Please select a training type.');
      return;
    }
    if (!name.trim()) {
    ...
    if (editingId) {
      setPackages(prev => prev.map(p => p.id === editingId
        ? { ...p, branch, name: name.trim(), trainer, trainingType, sessions: sessions.trim(), duration: duration.trim(), price: price.trim() }
        : p));
      setEditingId(null);
    } else {
      setPackages(prev => [...prev, {
        id: Date.now(), branch, name: name.trim(), trainer, trainingType, sessions: sessions.trim(), duration: duration.trim(), price: price.trim(), active: true,
      }]);
    }
    resetForm();
  };

  const handleUpdate = (item: TrainerPackageItem) => {
  ...
  const handleSetActive = (item: TrainerPackageItem, active: boolean) => {
    Alert.alert(
  ...
  const handleDelete = (item: TrainerPackageItem) => {
  ...
  const modalTitle = modalConfig.field === 'branch' ? 'Select Branch' : modalConfig.field === 'trainer' ? 'Select Trainer' : 'Select Training Type';
  const modalOptions = modalConfig.field === 'branch' ? BRANCH_OPTIONS : modalConfig.field === 'trainer' ? TRAINER_OPTIONS : TRAINING_TYPE_OPTIONS;
  const modalSelected = modalConfig.field === 'branch' ? branch : modalConfig.field === 'trainer' ? trainer : trainingType;
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {/* ── Add / Update Trainer Package ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>{editingId ? 'Update Trainer Package' : 'Add Trainer Packages'}</Text>
          </View>
          <View style={styles.form}>
            ...
            <SelectionField
              ...
              onPress={() => setModalConfig({ visible: true, field: 'trainer' })}
            />
            ...
            <SelectionField
              ...
              onPress={() => setModalConfig({ visible: true, field: 'trainingType' })}
            />
        ...
        {/* ── Active Trainer Package ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Active Trainer Package</Text>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator>
            <View>
              <View style={tbl.headerRow}>
                ...
                <Text style={[tbl.headerCell, { width: 100 }]}>Training Type</Text>
                <Text style={[tbl.headerCell, { width: 80 }]}>Session</Text>
              ...
              {loading
                ...
                : activePageData.map((p, i) => (
                  <View key={p.id} style={[tbl.dataRow, (activeStartIdx + i) % 2 === 1 && tbl.dataRowAlt]}>
                    ...
                    <Text style={[tbl.cell, { width: 100 }]} numberOfLines={1}>{p.trainingType}</Text>
        ...
        {/* ── Inactive Trainer Package ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Inactive Trainer Package</Text>
      ...
      <SelectionModal
        ...
        onSelect={(val: string) => {
          ...
          else if (modalConfig.field === 'trainer') setTrainer(val);
          else setTrainingType(val);
