...
// Physiotherapy Packages category code
const CATEGORY = 14;

const mapPackage = (p: any): PhysioPackageItem => ({
  id: p.id,
...
const PhysiotherapyPackages = () => {
  ...
  const handleAdd = () => {
    ...
    if (editingId) {
      setPackages(prev => prev.map(p => p.id === editingId
        ? { ...p, branch, name: name.trim(), sessions: sessions.trim(), duration: duration.trim(), price: price.trim() }
        : p));
      setEditingId(null);
    } else {
      setPackages(prev => [...prev, {
        id: Date.now(), branch, name: name.trim(), sessions: sessions.trim(), duration: duration.trim(), price: price.trim(), active: true,
      }]);
    }
    resetForm();
  };

  const handleUpdate = (item: PhysioPackageItem) => {
  ...
  const handleSetActive = (item: PhysioPackageItem, active: boolean) => {
    Alert.alert(
  ...
  const handleDelete = (item: PhysioPackageItem) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {/* ── Add / Update Physiotherapy Package ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>{editingId ? 'Update Physiotherapy Package' : 'Add Physiotherapy Package'}</Text>
          </View>
        ...
        {/* ── Active Physiotherapy Packages ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Active Physiotherapy Packages</Text>
        ...
        {/* ── Inactive Physiotherapy Packages ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Inactive Physiotherapy Packages</Text>
...
export default PhysiotherapyPackages;
