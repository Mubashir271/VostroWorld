...
// GX Packages category code
const CATEGORY = 15;

const classLabel = (c: any) => c?.class_name ?? c?.name ?? c?.title ?? '—';

const mapPackage = (p: any): GXPackageItem => ({
  id: p.id,
  branch: p.branches_name ?? '',
  className: p.package_name ?? '',
  trainer: [p.user_first_name, p.user_last_name].filter(Boolean).join(' '),
  sessions: String(p.session_count ?? ''),
  duration: String(p.duration ?? ''),
  bookingSpace: String(p.booking_capacity ?? ''),
...
const GXPackages = () => {
  ...
  const [gxClass, setGxClass] = useState<any>(null);
  ...
  const handleAdd = () => {
    if (!gxClass) {
      Alert.alert('Validation', 'Please select an available class.');
      return;
    }
    if (!price.trim()) {
    ...
    if (editingId) {
      setPackages(prev => prev.map(p => p.id === editingId
        ? { ...p, branch, className: classLabel(gxClass), price: price.trim(), duration: duration.trim() }
        : p));
      setEditingId(null);
    } else {
      setPackages(prev => [...prev, {
        id: Date.now(), branch, className: classLabel(gxClass), trainer: '', sessions: '', duration: duration.trim(), bookingSpace: '', price: price.trim(), active: true,
      }]);
    }
    resetForm();
  };

  const handleUpdate = (item: GXPackageItem) => {
    setEditingId(item.id);
    setBranch(item.branch);
    setGxClass(classes.find(c => classLabel(c) === item.className) ?? { class_name: item.className });
    setPrice(item.price);
    setDuration(item.duration);
  };

  const handleSetActive = (item: GXPackageItem, active: boolean) => {
    Alert.alert(
      active ? 'Activate Package' : 'Deactivate Package',
      `${active ? 'Activate' : 'Deactivate'} "${item.className}"?`,
      [
  ...
  const handleDelete = (item: GXPackageItem) => {
    Alert.alert('Delete Package', `Delete "${item.className}"?`, [
      { text: 'Cancel', style: 'cancel' },
      {
  ...
  const activePackages = packages.filter(p => p.active && (!searchActive.trim() || p.className.toLowerCase().includes(searchActive.trim().toLowerCase())));
  const inactivePackages = packages.filter(p => !p.active && (!searchInactive.trim() || p.className.toLowerCase().includes(searchInactive.trim().toLowerCase())));
  ...
  const classOptions = classes.map(c => ({ id: String(c.id), label: classLabel(c) }));
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {/* ── Add / Update GX Package ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>{editingId ? 'Update GX Package' : 'GX Package'}</Text>
          </View>
          <View style={styles.form}>
            ...
            <SelectionField
              ...
              value={gxClass ? classLabel(gxClass) : ''}
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Active GX Package</Text>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator>
            <View>
              <View style={tbl.headerRow}>
                ...
                <Text style={[tbl.headerCell, { width: 200 }]}>Class Name</Text>
                <Text style={[tbl.headerCell, { width: 120 }]}>Trainer</Text>
                <Text style={[tbl.headerCell, { width: 90 }]}>Session Count</Text>
                <Text style={[tbl.headerCell, { width: 80 }]}>Duration</Text>
                <Text style={[tbl.headerCell, { width: 100 }]}>Booking Space</Text>
              ...
              {loading
                ...
                : activePageData.map((p, i) => (
                  <View key={p.id} style={[tbl.dataRow, (activeStartIdx + i) % 2 === 1 && tbl.dataRowAlt]}>
                    ...
                    <Text style={[tbl.cell, { width: 200 }]} numberOfLines={1}>{p.className}</Text>
                    <Text style={[tbl.cell, { width: 120 }]} numberOfLines={1}>{p.trainer}</Text>
                    <Text style={[tbl.cell, { width: 90 }]} numberOfLines={1}>{p.sessions}</Text>
                    <Text style={[tbl.cell, { width: 80 }]} numberOfLines={1}>{`${p.duration} Days`}</Text>
                    <Text style={[tbl.cell, { width: 100 }]} numberOfLines={1}>{p.bookingSpace}</Text>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Inactive GX Package</Text>
      ...
      <SelectionModal
        ...
        selectedValue={gxClass ? classLabel(gxClass) : ''}
        onSelect={(val: string) => {
          setGxClass(classes.find(c => classLabel(c) === val) ?? null);
          setClassModal(false);
        }}
