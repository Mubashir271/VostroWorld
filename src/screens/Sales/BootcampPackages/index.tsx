...
// Bootcamp Packages category code
const CATEGORY = 7;

const mapPackage = (p: any): BootcampPackageItem => ({
  id: p.id,
...
const BootcampPackages = () => {
  ...
  const handleAdd = () => {
    ...
    if (editingId) {
      setPackages(prev => prev.map(p => p.id === editingId
        ? { ...p, branch, name: name.trim(), price: price.trim(), sessions: sessions.trim(), duration: duration.trim() }
        : p));
      setEditingId(null);
    } else {
      setPackages(prev => [...prev, {
        id: Date.now(), branch, name: name.trim(), price: price.trim(), sessions: sessions.trim(), duration: duration.trim(), active: true,
      }]);
    }
    resetForm();
  };

  const handleUpdate = (item: BootcampPackageItem) => {
  ...
  const handleSetActive = (item: BootcampPackageItem, active: boolean) => {
    Alert.alert(
  ...
  const handleDelete = (item: BootcampPackageItem) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {/* ── Add / Update Bootcamp Package ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>{editingId ? 'Update Bootcamp Package' : 'Add New Bootcamp Classes'}</Text>
          </View>
        ...
        {/* ── Active Bootcamp Package ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Active Bootcamp Package</Text>
        ...
        {/* ── Inactive Bootcamp Package ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Inactive Bootcamp Package</Text>
...
export default BootcampPackages;
