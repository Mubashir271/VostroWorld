...
// CFT / Academy category code
const CATEGORY = 11;

const mapPackage = (p: any): CFTPackageItem => ({
  id: p.id,
...
const CFTPackages = () => {
  ...
  const [packages, setPackages] = useState<CFTPackageItem[]>([]);
  ...
  const handleUpdate = (item: CFTPackageItem) => {
  ...
  const handleSetActive = (item: CFTPackageItem, active: boolean) => {
    Alert.alert(
  ...
  const handleDelete = (item: CFTPackageItem) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        {/* ── Add / Update CFT Package ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>{editingId ? 'Update CFT Package' : 'Add CFT Package'}</Text>
          </View>
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Active CFT Package</Text>
              ...
              {loading
                ...
                : activePageData.map((p, i) => (
                  <View key={p.id} style={[tbl.dataRow, (activeStartIdx + i) % 2 === 1 && tbl.dataRowAlt]}>
                    ...
                    <Text style={[tbl.cell, { width: 90 }]} numberOfLines={1}>{p.duration}</Text>
        ...
        {/* ── Inactive CFT Package ── */}
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Inactive CFT Package</Text>
